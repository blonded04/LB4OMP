...
#endif
    typename traits_t<T>::signed_t chunk, T nproc, T unit_id);
template <typename T>
...
template <typename T> struct dispatch_shared_infoXX_template {
  ...
  /* Batch chunk size used in FAC schedules */
  volatile UT chunk_size;
  /* Counter used in FAC */
  volatile UT counter;
  /* Queue counter used in FAC */
  volatile T q_counter;
  /* Batch index used in batched schedules */
  volatile UT batch;
  /*************BOLD***************/
  struct KMP_ALIGN(32) {
    volatile ST bold_m;
    volatile ST bold_n;
    volatile ST total_speed;
    volatile std::atomic<kmp_int64> bold_time;
  };
  /*************BOLD***************/
  /*************AWF/AF*************/
...
template <>
__forceinline kmp_int32 test_then_sub<kmp_int32>(volatile kmp_int32 *p,
                                                 kmp_int32 d) {
  kmp_int32 r;
  r = KMP_TEST_THEN_SUB32(p, d);
  return r;
}

template <>
__forceinline kmp_int64 test_then_sub<kmp_int64>(volatile kmp_int64 *p,
                                                 kmp_int64 d) {
  kmp_int64 r;
...
// Get time stamp in nanoseconds.
 static __forceinline kmp_int64 __kmp_get_nanos() {
    kmp_int64 ns =
